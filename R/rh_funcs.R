...
#' @param userAuthToken oauth2 token generated by rh_getAuthToken.
rh_getAccountID <- function(userAuthToken) {
...
#' @param userAuthToken oauth2 token generated by rh_getAuthToken.
rh_getAccountNumber <- function(userAuthToken) {
  header <- rh_createAuthHeader(userAuthToken)
...
rh_getPositions <- function(userAuthToken, accountNumber) {
  header <- rh_createAuthHeader(userAuthToken)
  positionsURL <- paste("https://api.robinhood.com/accounts/", accountNumber, "/positions/", sep = "")
...
#' @param  userAuthToken oauth2 token generated by rh_getAuthToken.
rh_getInstrumentURL <- function(ticker,userAuthToken) {
...
#' @param userAuthToken oauth2 token generated by rh_getAuthToken.
#' @param accountNumber account number generated by rh_getAccountNumber.
rh_createPositionsTable <- function(userAuthToken, accountNumber) {
  positionsResponse <- rh_getPositions(userAuthToken, accountNumber)
...
#' @param userAuthToken oauth2 token generated by rh_getAuthToken.
#' @param accountNumber account number generated by rh_getAccountNumber.
rh_getPortfolioEquity <- function(userAuthToken, accountNumber) {
  header <- rh_createAuthHeader(userAuthToken)
  portfolioURL <- paste("https://api.robinhood.com/accounts/", accountNumber, "/portfolio/", sep = "")
...
#' @param userAuthToken oauth2 token generated by rh_getAuthToken.
#' @param accountNumber account number generated by rh_getAccountNumber.
rh_getDayPL<-function(userAuthToken,accountNumber){
  header <- rh_createAuthHeader(userAuthToken)
  portfolioURL <- paste("https://api.robinhood.com/accounts/", accountNumber, "/portfolio/", sep = "")
  res <- httr::GET(portfolioURL, httr::add_headers(.headers=header))
...
rh_placeImmediateBuyOrder <- function(userAuthToken, accountNumber, ticker, orderType, orderPrice, numShares) {
  header <- rh_createAuthHeader(userAuthToken)
  creds = list(
    account = paste("https://api.robinhood.com/accounts/", accountNumber, "/", sep=""),
...
rh_placeStopBuyOrder <- function(userAuthToken, accountNumber, ticker, orderType, orderPrice, numShares, stopPrice) {
  header <- rh_createAuthHeader(userAuthToken)
  creds = list(
    account = paste("https://api.robinhood.com/accounts/", accountNumber, "/", sep=""),
    instrument = rh_getInstrumentURL(ticker),
...
#' @param numShares     number of shares. Numeric class integer only.
#' @param stopPrice     numeric stop price
rh_placeStopSellOrder <- function(userAuthToken, accountNumber, ticker, orderType, orderPrice, numShares, stopPrice) {
  header <- rh_createAuthHeader(userAuthToken)
  creds = list(
    account = paste("https://api.robinhood.com/accounts/", accountNumber, "/", sep=""),
    instrument = rh_getInstrumentURL(ticker),
...
#' @param orderPrice    numeric limit price (needed for market order because collared on price)
#' @param numShares     number of shares. Numeric class integer only.
rh_placeImmediateSellOrder <- function(userAuthToken, accountNumber, ticker, orderType, orderPrice, numShares) {
  header <- rh_createAuthHeader(userAuthToken)
  creds = list(
    account = paste("https://api.robinhood.com/accounts/", accountNumber, "/", sep=""),
    instrument = rh_getInstrumentURL(ticker),
...
#'
#' Get current positions and their market value in a data.frame.
rh_getBalances<-function(){
  positions<-rh_createPositionsTable(userAuthToken,accountNumber)
  shares<-as.numeric(as.character(positions$quantity))
  idx_owned<-which(shares>0)
  positions<-positions[idx_owned,]
  tickers<-rownames(positions)
  quotes<-rh_quote(tickers)
  prices<-quotes$last_extended_hours_trade_price
  NA_prices<-which(is.na(quotes$last_extended_hours_trade_price)==TRUE)

  if(length(NA_prices)>0){
    new_quotes<-rh_quote(tickers[NA_prices])
    prices[NA_prices]<-new_quotes$last_trade_price
  }
  shares<-shares[idx_owned]
  prices<-as.numeric(prices)
  market_value<-sum(shares*prices)
  equity<-as.numeric(rh_getPortfolioEquity(userAuthToken,accountNumber))
  cash_balance<-equity-market_value
  market_value<-shares*prices
  market_value<-c(cash_balance,market_value)
  tickers<-c("CASH",tickers)
  shares<-c("NA",shares)
  prices<-c("NA",prices)
  portfolio<-data.frame(Ticker=tickers,Shares=shares,"Last Price"=prices,"Market Value"=market_value)
  return(portfolio)
}
...
#' @param ticker  Equity ticker symbol. All caps string.
rh_getFundamentals<-function(ticker){
...
#' @param userAuthToken oauth2 token generated by rh_getAuthToken.
rh_getRecentOrders <- function(userAuthToken) {
  header <- rh_createAuthHeader(userAuthToken)
...
#' @param userAuthToken oauth2 token generated by rh_getAuthToken.
rh_getOptionsOrders <- function(userAuthToken) {
  header <- rh_createAuthHeader(userAuthToken)
...
#' @param userAuthToken oauth2 token generated by rh_getAuthToken.
#' @param accountNumber account number generated by rh_getAccountNumber.
rh_getOptionsPositions <- function(userAuthToken, accountNumber) {
  header <- rh_createAuthHeader(userAuthToken)
...
rh_historicals <- function(symbols, interval = "day",
                           ...
                           keep_meta = FALSE, to_xts = TRUE,userAuthToken=userAuthToken){
  ...
  hist_list <- lapply(X = symbols,
                      function(x){rh_historicals_one(symbol = x,
                                                     interval = interval,
                                                     span = span,
                                                     bounds = bounds,userAuthToken = userAuthToken)})
...
#' @param bounds extended|regular|trading
#' @param userAuthToken oauth2 token generated by rh_getAuthToken.
rh_historicals_one <- function(symbol, interval = interval, span = span, bounds = bounds,userAuthToken=userAuthToken){
